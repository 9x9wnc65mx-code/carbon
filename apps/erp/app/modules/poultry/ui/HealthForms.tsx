...
export function ClinicalEventForm({ diseases, defaultDateTime }: { diseases: DiseaseOption[]; defaultDateTime: string }) {
  return (
    <ValidatedForm validator={clinicalEventValidator} method="post" className="grid grid-cols-1 gap-3 md:grid-cols-2">
      ...
      <label className="grid gap-1 text-sm"><span><Trans>Event type</Trans></span><select name="eventType" defaultValue="Clinical Observation" className={inputClass}><option>Clinical Observation</option><option>Diagnosis</option><option>Necropsy</option><option>Mortality Investigation</option><option>Follow-up</option><option>Other</option></select></label>
      <label className="grid gap-1 text-sm"><span><Trans>Body system</Trans></span><select name="bodySystem" defaultValue="Mixed" className={inputClass}><option>Respiratory</option><option>Enteric</option><option>Locomotor</option><option>Nervous</option><option>Systemic</option><option>Reproductive</option><option>Mixed</option><option>Other</option></select></label>
      <label className="grid gap-1 text-sm"><span><Trans>Severity</Trans></span><select name="severity" defaultValue="Moderate" className={inputClass}><option>Mild</option><option>Moderate</option><option>Severe</option><option>Critical</option></select></label>
      ...
      <label className="grid gap-1 text-sm"><span><Trans>Diagnosis role</Trans></span><select name="diagnosisRole" defaultValue="Suspected" className={inputClass}><option>Suspected</option><option>Confirmed</option><option>Differential</option><option>Ruled Out</option></select></label>
...
export function ClinicalEventResolutionForm({ eventId, defaultDateTime }: { eventId: string; defaultDateTime: string }) {
  return (
    <ValidatedForm validator={clinicalEventResolutionValidator} method="post" className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-2">
      ...
      <label className="grid gap-1 text-sm"><span><Trans>Case status</Trans></span><select name="status" defaultValue="Monitoring" className={inputClass}><option>Monitoring</option><option>Resolved</option><option>Closed</option></select></label>
...
export function TreatmentCourseForm({ drugs, clinicalEvents, defaultDateTime }: { drugs: DrugOption[]; clinicalEvents: ClinicalOption[]; defaultDateTime: string }) {
  ...
  return (
    <ValidatedForm validator={treatmentCourseValidator} method="post" className="grid grid-cols-1 gap-3 md:grid-cols-2">
      ...
      <label className="grid gap-1 text-sm"><span><Trans>Dose unit</Trans></span><input name="doseUnit" dir="ltr" className={inputClass} placeholder="mg/kg BW" /></label>
      <label className="grid gap-1 text-sm"><span><Trans>Frequency</Trans></span><input name="frequency" className={inputClass} placeholder="Once daily" /></label>
...
export function TreatmentAdministrationForm({ course, defaultDateTime }: { course: Record<string, any>; defaultDateTime: string }) {
  return (
    <ValidatedForm validator={treatmentAdministrationValidator} method="post" className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-2">
      ...
      <label className="grid gap-1 text-sm"><span><Trans>Actual dose</Trans></span><input name="doseValue" type="number" min="0" step="0.001" dir="ltr" defaultValue={course.doseValue ?? ""} className={inputClass} /></label>
      <label className="grid gap-1 text-sm"><span><Trans>Dose unit</Trans></span><input name="doseUnit" dir="ltr" defaultValue={course.doseUnit ?? ""} className={inputClass} /></label>
...
export function TreatmentStatusForm({ courseId }: { courseId: string }) {
  return (
    <ValidatedForm validator={treatmentStatusValidator} method="post" className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-[180px_1fr_auto]">
      ...
      <select name="status" defaultValue="Completed" className={inputClass}><option>Completed</option><option>Stopped</option><option>Cancelled</option></select>
