...
| `.` (timezone) | `getCompanyTimeZone(db, companyId)` / `getLocationTimeZone(db, locationId, companyId)` (`src/timezone.ts`) — business-timezone resolvers, overloaded for Supabase client or Kysely handle; `AnyPostgresClient` + `isKysely` guard (`src/utils.ts`) for writing such overloads. SQL siblings: `company_today(companyId)` / `location_today(locationId, companyId)` replace `CURRENT_DATE` for business dates in DB functions (SECURITY INVOKER — callers must be SECURITY DEFINER or service-role). ERP routes should prefer the Redis-cached wrappers in `~/modules/shared/timezone.server` |
