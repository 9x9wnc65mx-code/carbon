...
| Editable currency (`INPUT_FORMAT.money` / `.price`) | same as its kind above | an input formats with the SAME digits it displays with, padding included — an empty cost reads "$0.00". react-aria's blur commit is literally `setNumberValue(parse(format(x)))`, so this is not decoration — it is what a typed amount is STORED at. `.money` commits 300.22121 as 300.22 in USD and 63.4 as 63 in JPY; `.price` commits 0.00123 as 0.00123, which is the whole point of the price kind |

Call sites pick a kind (`formatMoney/Price/Percent/Quantity`, the
`useCurrencyFormatter` (with `price`/`compact`/`wholeUnits` naming the
